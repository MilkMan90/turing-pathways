...
class PathsForm extends Component {
  ...
  render() {
    let saveEnabled;
    if(this.formIsOK()){
      saveEnabled = "save-city-button glow"
    } else {
      saveEnabled = "save-city-button lock"
    }
    let pathTitle;
    ...
    return (
      <div className='single-path-form'>
        ...
        <label className="country-select">
          Country
          <select value={this.props.country} onChange={(e)=>this.props.updatePath(e.target.value, "country")}>
